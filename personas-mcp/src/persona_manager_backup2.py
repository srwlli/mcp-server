...
class PersonaManager:
    ...
    def load_persona(self, name: str) -> PersonaDefinition:
        ...
        Args:
            name: Persona name (e.g., 'mcp-expert' or 'mcp-expert:coderef')
        ...
        # Determine file path (check base/ and specialized/ directories)
        # First try base directory
        persona_file = self.personas_dir / "base" / f"{name}.json"

        # If not in base, check specialized directory
        if not persona_file.exists():
            persona_file = self.personas_dir / "specialized" / f"{name}.json"

        if not persona_file.exists():
            raise FileNotFoundError(
                f"Persona '{name}' not found in base/ or specialized/ directories. "
    ...
    def list_available_personas(self) -> list[str]:
        """
        List all available persona names from both base/ and specialized/ directories.
        ...
        specialized_dir = self.personas_dir / "specialized"
        if specialized_dir.exists():
            for file in specialized_dir.glob("*.json"):
                personas.append(file.stem)
