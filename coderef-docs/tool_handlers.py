...
async def handle_generate_foundation_docs(arguments: dict) -> list[TextContent]:
    ...
    if resources['resources_available']:
        ...
        # WO-DOCS-SCANNER-INTEGRATION-001: IMPL-006 - Module Dependencies instructions
        result += "RELATIONSHIP DATA INTEGRATION (v4.1.0):\n"
        result += "For ARCHITECTURE.md, add Module Dependencies section using import/export data:\n\n"
        result += "Python aggregation examples:\n"
        result += "```python\n"
        result += "# Aggregate imports across all elements\n"
        result += "import_counts = {}\n"
        result += "for e in elements:\n"
        result += "    for imp in e.get('imports', []):\n"
        result += "        source = imp.get('source')\n"
        result += "        if source:\n"
        result += "            import_counts[source] = import_counts.get(source, 0) + 1\n\n"
        result += "# Identify high-dependency modules (>= 5 usages)\n"
        result += "high_deps = {k: v for k, v in import_counts.items() if v >= 5}\n"
        result += "high_deps = dict(sorted(high_deps.items(), key=lambda x: x[1], reverse=True))\n\n"
        result += "# Aggregate exports by file\n"
        result += "exports_by_file = {}\n"
        result += "for e in elements:\n"
        result += "    file = e.get('file')\n"
        result += "    exports = e.get('exports', [])\n"
        result += "    if file and exports:\n"
        result += "        exports_by_file.setdefault(file, []).extend(exports)\n"
        result += "```\n\n"
        result += "ARCHITECTURE.md SECTIONS:\n"
        result += "Add '## Module Dependencies' section with:\n"
